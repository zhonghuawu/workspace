...
void Decomposition::run(){
	...
	list<int>::iterator it=classes[labels_number-1].begin();
	vector<double> mean(datas[*it].begin(),datas[*it].end());
	for(++it;it!=classes[labels_number-1].end();it++){
		transform(datas[*it].begin(),datas[*it].end(),mean.begin(),mean.begin(),plus<double>());
	}
	means[labels_number-1].resize(mean.size(),0.0);
	transform(mean.begin(),mean.end(),means[labels_number-1].begin(),bind2nd(divides<double>(),lens[labels_number-1]));
}
